...
export default function InboundTranscripts() {
  ...
  // Load inbound transcripts
  const loadTranscripts = async () => {
    try {
      setIsLoading(true);
      setError(null);
      
      const response = await getInboundTranscripts();
      console.log('📥 Inbound transcripts response:', response);
      ...
      if (response && response.success && response.transcripts && Array.isArray(response.transcripts)) {
        const processedTranscripts = response.transcripts.map((transcript: any) => ({
          requestUuid: transcript.requestUuid || `inbound_${Date.now()}_${Math.random()}`,
      ...
      console.error('Error loading inbound transcripts:', error);
      setError('Failed to load inbound transcripts');
      setTranscripts([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const loadDetailedTranscript = async (requestUuid: string) => {
    try {
      const response = await getInboundTranscript(requestUuid);
  ...
  const connectWebSocket = () => {
    ...
    ws.onopen = () => {
      console.log('✅ WebSocket connected for inbound transcripts');
      setIsConnected(true);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'transcript_update') {
          ...
          // Only process if it's an inbound call (we can check callType if available)
          if (data.callType === 'inbound' || !data.callType) {
        ...
        if (data.type === 'summary' && (data.callType === 'inbound' || !data.callType)) {
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="animate-fade-in">
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-3xl font-bold">Inbound Call Transcripts</h1>
          ...
          <p className="text-muted-foreground">
            View and manage inbound call transcripts from Plivo + OpenAI
          </p>
        </div>
        ...
        <Card className="glass-card animate-slide-up delay-200">
          <CardHeader>
            <CardTitle>Inbound Call Transcripts</CardTitle>
          ...
          <CardContent>
            {isLoading ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">Loading inbound transcripts...</p>
              </div>
            ) : transcripts.length === 0 ? (
              <div className="text-center py-8">
                <PhoneIncoming className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                <p className="text-muted-foreground">No inbound transcripts found</p>
              </div>
            ) : (
                            ...
                            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                              <DialogHeader>
                                <DialogTitle className="flex items-center space-x-2">
                                  ...
                                  <span>Inbound Call Transcript - {selectedTranscript?.phoneNumber}</span>
                                </DialogTitle>
                              </DialogHeader>
                                        ...
                                        <div className="space-y-4">
                                          {selectedTranscript.conversation.map((message, index) => (
                                            <div key={index} className="flex flex-col space-y-1">
                                              ...
                                              <div className="text-sm pl-4 border-l-2 border-blue-200 py-2">
                                                {message.text}
                                              </div>
                                            </div>
                                          ))}
